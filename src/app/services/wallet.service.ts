...
export class WalletService {
    ...
    if (transaction.is_send === 'true' && transaction.block.link_as_account) {
      ...
      if (this.appSettings.settings.minimumReceive) {
        ...
        if (txAmount.gt(minAmount)) {
          ...
          console.log(`发现低于最小接收量的新待处理块: `, transaction.amount, this.appSettings.settings.minimumReceive);
        }
      } else {
        this.addPendingBlock(walletAccount.id, transaction.hash, txAmount);
      }
  ...
  async addWalletAccount(accountIndex: number|null = null, reloadBalances: boolean = true) {
    ...
    if (this.wallet.type === 'privateKey') {
      throw new Error(`无法在私钥模式下添加另一个帐户`);
  ...
  async removeWalletAccount(accountID: string) {
    ...
    if (!walletAccount) throw new Error(`账户不在钱包里`);

    const walletAccountIndex = this.wallet.accounts.findIndex(a => a.id === accountID);
    if (walletAccountIndex === -1) throw new Error(`账户不在钱包里`);
  ...
  async processPendingBlocks() {
    ...
    if (newHash) {
      ...
      this.notifications.sendSuccess(`成功接收 ${receiveAmount.isZero() ? '' : receiveAmount.toFixed(6)} FFD!`);
      ...
      return this.notifications.sendError(`执行接收事务时出现问题，请手动尝试!`);
    }
