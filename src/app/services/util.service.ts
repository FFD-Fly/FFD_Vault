...
function getAccountPublicKey(account) {
  if (account.length == 64) {
    if(!account.startsWith('ffd_1') && !account.startsWith('ffd_3')) {
      throw new Error(`无效的 FFD 帐户`);
    }
  } else if (account.length == 65) {
    if(!account.startsWith('ffd_1') && !account.startsWith('ffd_3')) {
      throw new Error(`无效的 FF 帐户`);
    }
  } else {
    throw new Error(`无效的 FFD 帐户`);
  }
  ...
  if (!isValid) throw new Error(`无效的 FFD 帐户`);
  ...
  if (!equal_arrays(hash_uint4, uint8ToUint4(blake_hash))) throw new Error(`校验和不正确`);
