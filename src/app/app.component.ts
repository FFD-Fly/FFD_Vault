...
export class AppComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    // New for v19: Patch saved xrb_ prefixes to ffd_
    ...
    if (this.wallet.locked && this.walletService.hasPendingTransactions()) {
      this.notifications.sendWarning(`新的传入交易 - 解锁钱包以接收它!`, { length: 0, identifier: '待锁定' });
    }
    ...
    // Listen for an ffd: protocol link, triggered by the desktop application
    window.addEventListener('protocol-load', (e: CustomEvent) => {
      const protocolText = e.detail;
      const stripped = protocolText.split('').splice(4).join(''); // Remove ffd:
      if (stripped.startsWith('ffd_')) {
    ...
    setInterval(() => {
      ...
      if (this.inactiveSeconds >= this.settings.settings.lockInactivityMinutes * 60) {
        this.walletService.lockWallet();
        this.notifications.sendSuccess(`钱包锁定后 ${this.settings.settings.lockInactivityMinutes} 不活动分钟`);
      }
    }, 1000);
    ...
    try {
      ...
      this.notifications.sendWarning(`检索最新 FFD 价格时出现问题。 确保您的 AdBlocker 在此页面上被禁用，然后重新加载以查看准确的 FIAT 值。`, { length: 0, identifier: `price-adblock` });
    }
  ...
  /*
    ...
    (Your wallet, address book, rep list, etc) and updates them to ffd_ prefix for v19 RPC
   */
  async patchXrbToNanoPrefixData() {
    ...
    await this.walletService.patchOldSavedData(); // Change saved xrb_ addresses to ffd_
    this.addressBook.patchXrbPrefixData();
    this.representative.patchXrbPrefixData();

    this.settings.setAppSetting('钱包版本', 2); // Update wallet version so we do not patch in the future.
  }
  ...
  performSearch() {
    ...
    if (searchData.startsWith('xrb_') || searchData.startsWith('ffd_')) {
      this.router.navigate(['account', searchData]);
    } else if (searchData.length === 64) {
      this.router.navigate(['交易', searchData]);
    } else {
      this.notifications.sendWarning(`FFD 帐户或交易哈希无效!`)
    }
    this.searchData = '';
  }
  ...
  retryConnection() {
    ...
    this.notifications.sendInfo(`尝试重新连接到 FFD 节点`);
  }

  async updateFiatPrices() {
    const displayCurrency = this.settings.getAppSetting(`显示货币`) || 'USD';
