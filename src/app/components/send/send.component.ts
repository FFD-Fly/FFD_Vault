...
export class SendComponent implements OnInit {
  ...
  amounts = [
    { name: 'FFD (1 FFD)', shortName: 'FFD', value: 'mnano' },
  ...
  // An update to the FFD amount, sync the fiat value
  syncFiatPrice() {
  ...
  // An update to the fiat amount, sync the ffd value based on currently selected denomination
  syncNanoPrice() {
  ...
  async sendTransaction() {
    ...
    if (!from) return this.notificationService.sendError(`未找到来自帐户`);
    ...
    if (this.amount < 0 || rawAmount.lessThan(0)) return this.notificationService.sendWarning(`金额无效`);
    if (nanoAmount.lessThan(1)) return this.notificationService.sendWarning(`节点将忽略小于 1 纳米的事务。 发送至少 1 纳米的原始数量.`);
    if (from.balanceBN.minus(rawAmount).lessThan(0)) return this.notificationService.sendError(`从账户没有足够的 FFD`);
  ...
  async confirmTransaction() {
    ...
    if (!walletAccount) throw new Error(`无法在钱包中找到发送账户`);
    if (this.walletService.walletIsLocked()) return this.notificationService.sendWarning(`钱包必须解锁`);
    ...
    try {
      ...
      if (newHash) {
        this.notificationService.sendSuccess(`发送成功 ${this.amount} ${this.selectedAmount.shortName}!`);
        ...
        if (!this.walletService.isLedgerWallet()) {
          this.notificationService.sendError(`发送您的交易时出错，请重试.`)
        }
      }
    } catch (err) {
      this.notificationService.sendError(`发送您的交易时出错: ${err.message}`)
    }
