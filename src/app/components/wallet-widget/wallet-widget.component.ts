...
export class WalletWidgetComponent implements OnInit {
  ...
  async lockWallet() {
    ...
    if (!this.wallet.password) {
      return this.notificationService.sendWarning(`您必须在钱包上设置密码 - 目前为空白!`);
    }
    const locked = await this.walletService.lockWallet();
    if (locked) {
      this.notificationService.sendSuccess(`钱包锁定`);
    } else {
      this.notificationService.sendError(`无法锁定钱包`);
    }
  }
  ...
  async unlockWallet() {
    ...
    if (unlocked) {
      this.notificationService.sendSuccess(`钱包解锁`);
      this.modal.hide();
    } else {
      this.notificationService.sendError(`密码无效，请重试！`);
    }
