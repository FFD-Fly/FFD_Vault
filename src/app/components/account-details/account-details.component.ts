...
export class AccountDetailsComponent implements OnInit, OnDestroy {
  ...
  async saveRepresentative() {
    ...
    if (!valid || valid.valid !== '1') return this.notifications.sendWarning(`帐户 ID 不是有效帐户`);

    try {
      const changed = await this.nanoBlock.generateChange(this.walletAccount, repAccount, this.wallet.isLedgerWallet());
      if (!changed) {
        this.notifications.sendError(`更换代表时出错，请重试`);
        return;
      }
    } catch (err) {
    ...
    this.notifications.sendSuccess(`成功更换代表`);
  }
  ...
  async saveAddressBook() {
    ...
    if (!addressBookName) {
      ...
      if (this.addressBookEntry) {
        ...
        this.notifications.sendSuccess(`已成功删除通讯录条目!`);
    ...
    this.notifications.sendSuccess(`保存的通讯录条目!`);
  ...
  copied() {
    this.notifications.sendSuccess(`已成功复制到剪贴板！`);
  }
